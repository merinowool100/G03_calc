...
function updateBestRecord() {
    ...
    let bestRecord2 = localStorage.getItem('bestRecord2');

    if (bestRecord2 === null) {
        bestRecord2 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord2 = parseInt(bestRecord2, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord2 || bestRecord2 === 0) {
        localStorage.setItem('bestRecord2', currentRecord);  // 新しいbestRecordを保存
        bestRecord2 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord2);  // ベストレコードを表示
}

function displayBestRecord(bestRecord2) {
    if (isNaN(bestRecord2)) {
        bestRecord2 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord2 % 360000) / 6000);
    let seconds = Math.floor((bestRecord2 % 6000) / 100);
    let hundredths = bestRecord2 % 100;
...
window.addEventListener("load", () => {
    const bestRecord2 = localStorage.getItem('bestRecord2');
    if (bestRecord2) {
        displayBestRecord(bestRecord2); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord2 = localStorage.getItem("bestRecord2");
    if (bestRecord2 === null || milliseconds < bestRecord2) {
        message = "New record!!";
