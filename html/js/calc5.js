...
function updateBestRecord() {
    ...
    let bestRecord5 = localStorage.getItem('bestRecord5');

    if (bestRecord5 === null) {
        bestRecord5 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord5 = parseInt(bestRecord5, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord5 || bestRecord5 === 0) {
        localStorage.setItem('bestRecord5', currentRecord);  // 新しいbestRecordを保存
        bestRecord5 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord5);  // ベストレコードを表示
}

function displayBestRecord(bestRecord5) {
    if (isNaN(bestRecord5)) {
        bestRecord5 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord5 % 360000) / 6000);
    let seconds = Math.floor((bestRecord5 % 6000) / 100);
    let hundredths = bestRecord5 % 100;
...
window.addEventListener("load", () => {
    const bestRecord5 = localStorage.getItem('bestRecord5');
    if (bestRecord5) {
        displayBestRecord(bestRecord5); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord5 = localStorage.getItem("bestRecord5");
    if (bestRecord5 === null || milliseconds < bestRecord5) {
        message = "New record!!";
