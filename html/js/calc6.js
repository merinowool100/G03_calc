...
function createQ() {
    for (let i = 0; i < 10; i++) {
        ...
        } while ((num1 < 11) || num1 < num2 || num2 === 5);
...
function updateBestRecord() {
    ...
    let bestRecord6 = localStorage.getItem('bestRecord6');

    if (bestRecord6 === null) {
        bestRecord6 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord6 = parseInt(bestRecord6, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord6 || bestRecord6 === 0) {
        localStorage.setItem('bestRecord6', currentRecord);  // 新しいbestRecordを保存
        bestRecord6 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord6);  // ベストレコードを表示
}

function displayBestRecord(bestRecord6) {
    if (isNaN(bestRecord6)) {
        bestRecord6 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord6 % 360000) / 6000);
    let seconds = Math.floor((bestRecord6 % 6000) / 100);
    let hundredths = bestRecord6 % 100;
...
window.addEventListener("load", () => {
    const bestRecord6 = localStorage.getItem('bestRecord6');
    if (bestRecord6) {
        displayBestRecord(bestRecord6); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord6 = localStorage.getItem("bestRecord6");
    if (bestRecord6 === null || milliseconds < bestRecord6) {
        message = "New record!!";
