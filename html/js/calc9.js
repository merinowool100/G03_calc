...
function updateBestRecord() {
    ...
    let bestRecord9 = localStorage.getItem('bestRecord9');

    if (bestRecord9 === null) {
        bestRecord9 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord9 = parseInt(bestRecord9, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord9 || bestRecord9 === 0) {
        localStorage.setItem('bestRecord9', currentRecord);  // 新しいbestRecordを保存
        bestRecord9 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord9);  // ベストレコードを表示
}

function displayBestRecord(bestRecord9) {
    if (isNaN(bestRecord9)) {
        bestRecord9 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord9 % 360000) / 6000);
    let seconds = Math.floor((bestRecord9 % 6000) / 100);
    let hundredths = bestRecord9 % 100;
...
window.addEventListener("load", () => {
    ...
    if (bestRecord9) {
        displayBestRecord(bestRecord9); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
