...
function updateBestRecord() {
    ...
    let bestRecord7 = localStorage.getItem('bestRecord7');

    if (bestRecord7 === null) {
        bestRecord7 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord7 = parseInt(bestRecord7, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord7 || bestRecord7 === 0) {
        localStorage.setItem('bestRecord7', currentRecord);  // 新しいbestRecordを保存
        bestRecord7 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord7);  // ベストレコードを表示
}

function displayBestRecord(bestRecord7) {
    if (isNaN(bestRecord7)) {
        bestRecord7 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord7 % 360000) / 6000);
    let seconds = Math.floor((bestRecord7 % 6000) / 100);
    let hundredths = bestRecord7 % 100;
...
window.addEventListener("load", () => {
    const bestRecord7 = localStorage.getItem('bestRecord7');
    if (bestRecord7) {
        displayBestRecord(bestRecord7); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord7 = localStorage.getItem("bestRecord7");
    if (bestRecord7 === null || milliseconds < bestRecord7) {
        message = "New record!!";
