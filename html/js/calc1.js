...
function updateBestRecord() {
    ...
    let bestRecord1 = localStorage.getItem('bestRecord1');

    if (bestRecord1 === null) {
        bestRecord1 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord1 = parseInt(bestRecord1, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord1 || bestRecord1 === 0) {
        localStorage.setItem('bestRecord1', currentRecord);  // 新しいbestRecordを保存
        bestRecord1 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord1);  // ベストレコードを表示
}

function displayBestRecord(bestRecord1) {
    if (isNaN(bestRecord1)) {
        bestRecord1 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord1 % 360000) / 6000);
    let seconds = Math.floor((bestRecord1 % 6000) / 100);
    let hundredths = bestRecord1 % 100;
...
window.addEventListener("load", () => {
    const bestRecord1 = localStorage.getItem('bestRecord1');
    if (bestRecord1) {
        displayBestRecord(bestRecord1); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord1 = localStorage.getItem("bestRecord1");
    if (bestRecord1 === null || milliseconds < bestRecord1) {
        message = "New record!!";
