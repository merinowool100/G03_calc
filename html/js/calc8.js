...
function updateBestRecord() {
    ...
    let bestRecord8 = localStorage.getItem('bestRecord8');

    if (bestRecord8 === null) {
        bestRecord8 = 0;  // bestRecordがnullの場合は0に設定
    } else {
        bestRecord8 = parseInt(bestRecord8, 10);  // 整数として扱う
    }

    // 現在の記録が最良記録よりも短ければ更新する
    if (currentRecord < bestRecord8 || bestRecord8 === 0) {
        localStorage.setItem('bestRecord8', currentRecord);  // 新しいbestRecordを保存
        bestRecord8 = currentRecord;  // 変数に最良記録を格納
    }

    displayBestRecord(bestRecord8);  // ベストレコードを表示
}

function displayBestRecord(bestRecord8) {
    if (isNaN(bestRecord8)) {
        bestRecord8 = 0;  // 万が一bestRecordがNaNであれば0に設定
    }

    // 時間としてフォーマットする
    let minutes = Math.floor((bestRecord8 % 360000) / 6000);
    let seconds = Math.floor((bestRecord8 % 6000) / 100);
    let hundredths = bestRecord8 % 100;
...
window.addEventListener("load", () => {
    const bestRecord8 = localStorage.getItem('bestRecord8');
    if (bestRecord8) {
        displayBestRecord(bestRecord8); // ページロード時にベストレコード表示
    } else {
        displayBestRecord(0);
    }
});
...
function screen_lock() {
    ...
    const bestRecord8 = localStorage.getItem("bestRecord8");
    if (bestRecord8 === null || milliseconds < bestRecord) {
        message = "New record!!";
